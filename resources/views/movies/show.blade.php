...
<div class="container py-3">
  <div class="row g-4">
    <div class="col-md-4">
      <img src="{{ $movie->poster_url ?? asset('images/no-poster.png') }}" class="img-fluid rounded" alt="{{ $movie->title }}">
    ...
    <div class="col-md-8">
      ...
      <div class="text-muted mb-2">Khởi chiếu: {{ \Illuminate\Support\Carbon::parse($movie->release_date)->format('d/m/Y') }}</div>
      <p>{{ $movie->description }}</p>
      <div class="d-flex align-items-center gap-2 mt-3">
        <form method="get" class="d-flex gap-2">
          <input type="date" name="date" class="form-control" value="{{ $day->format('Y-m-d') }}">
          <button class="btn btn-outline-secondary"><i class="bi bi-calendar-event"></i> Lọc suất</button>
        </form>
      </div>
        ...
        <div class="list-group">
          @foreach($showtimes as $st)
            <div class="list-group-item d-flex justify-content-between align-items-center">
              ...
              <a class="btn btn-primary" href="{{ route('tickets.create',$st->id) }}">
                Chọn ghế
              </a>
            </div>
          @endforeach
